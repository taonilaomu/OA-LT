...
public class LogoutServlet extends HttpServlet {
    ...
    public static final String prefix = "LogoutServlet  ";
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        logger.info(prefix+login.toString());
        login.setLogcount(login.getLogcount() + 1);
        login.setLogouttime(new Date());
        try {
            LoginDto.saveLog(login);
            System.out.println(prefix + "成功插入" + login.toString());
        } catch (SQLException e) {
            throw new RuntimeException(e);
        }
        session.invalidate();
        logger.info(prefix+"退出了");
